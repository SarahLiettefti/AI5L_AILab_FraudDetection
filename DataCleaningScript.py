...
class pre_processing:
    def __init__(self, report=False, random_state=0, one_hot_encoder=True):
        ...
        # Data transformation
        self.raw_data["TransactionStartTime"] = pd.to_datetime(
            self.raw_data["TransactionStartTime"]
        )
        self.X_test["TransactionStartTime"] = pd.to_datetime(
            self.X_test["TransactionStartTime"]
        )
        ...
        self.data = self.delete_col_unique_val(
            self.data, report
        ...
        self.low_cardinality_cols = [
            ...
            if self.data[cname].nunique() < 15 and self.data[cname].dtype == "object"
        ]
        ...
        for col in self.l_col_str:
            ...
            self.X_test[["dc", "new_col"]] = self.X_test[col].str.split(
                "_", expand=True
            )
        ...
        self.cat_cols = [
            col for col in self.train_X.columns if self.train_X[col].dtype == "object"
        ]  # liste of obejct columns
        self.cat_cols.append(
            "PricingStrategy"
        )  # pcq mm si c'est un chiffre il faut le considérer comme une catégorie
        ...
        if one_hot_encoder:
            ...
            self.train_X[self.low_cardinality_cols] = self.train_X[
                self.low_cardinality_cols
            ].astype(str)
            self.val_X[self.low_cardinality_cols] = self.val_X[
                self.low_cardinality_cols
            ].astype(str)

            OH_encoder = OneHotEncoder(handle_unknown="ignore", sparse_output=False)
            OH_cols_train = pd.DataFrame(
                OH_encoder.fit_transform(self.train_X[self.low_cardinality_cols])
            )
            OH_cols_valid = pd.DataFrame(
                OH_encoder.transform(self.val_X[self.low_cardinality_cols])
            )
    ...
    def delete_col_unique_val(self, X, report=False):
        ...
        if report:
            print(
                f"The colomns : {self.cols_unique_value} have only one value and have been dropped"
            )
